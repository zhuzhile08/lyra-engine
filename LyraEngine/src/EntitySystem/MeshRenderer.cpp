...
// mesh renderer
MeshRenderer::MeshRenderer(const Mesh* const mesh)
	 : m_mesh(mesh), 
	...
	m_indexBuffer(sizeof(m_mesh->indices()[0]) * m_mesh->indices().size(), VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU)
{
	...
	vertexStagingBuffer.copy_data(m_mesh->vertices().data());
	// copy the buffer
	...
	indexStagingBuffer.copy_data(m_mesh->indices().data());
	// copy the buffer
	m_indexBuffer.copy(indexStagingBuffer);
}

void MeshRenderer::update() const noexcept {
