...
public:
	...
	template <class Ty, class... Args> Entity& addComponent(Args&&... args) {
		auto c = new (ecs::addComponent(typeid(Ty), sizeof(Ty), m_id)) Ty(std::forward<Args>(args)...);
	...
	template <class Ty> Entity& removeComponent() {
		ecs::removeComponent(typeid(Ty), m_id);
		return *this;
	}
	...
	template <class Ty> NODISCARD Ty& component() {
		if (!ecs::containsComponent(typeid(Ty), m_id)) {
			addComponent<Ty>();
		...
		return *reinterpret_cast<Ty*>(ecs::component(typeid(Ty), m_id));
	}
	template <class Ty> NODISCARD const Ty& component() const {
		if (!ecs::containsComponent(typeid(Ty), m_id)) { // bypass const
			auto c = new (ecs::addComponent(typeid(Ty), sizeof(Ty), m_id)) Ty();
			if constexpr (std::is_base_of_v<BasicComponent, Ty>) c->entity = this;
		}
	...
	template <class Ty> NODISCARD bool containsComponent() const {
		return ecs::containsComponent(typeid(Ty), m_id);
	}
