 ...
 * 
 * @brief Sparse Map implementation
 * 
 * @date 2024-02-24
 * 
...
public:
	...
	using wrapper = SparseMap;

	constexpr SparseMap() : m_buckets(2) { } 
	constexpr SparseMap(size_type bucketCount) : m_buckets(std::max(bucketCount, size_type(2))) { } 
	template <class It> constexpr SparseMap(It first, It last, size_type bucketCount = 0) : m_buckets(bucketCount) {  // set to 0 for default evaluation
		insert(first, last);
	}
	constexpr SparseMap(std::initializer_list<value_type> ilist, size_type bucketCount = 0) : m_buckets(bucketCount) { // set to 0 for default evaluation
		insert(ilist.begin(), ilist.end());
	} 
	constexpr SparseMap(const SparseMap& other) : m_array(other.m_array), m_buckets(other.m_buckets), m_lookup(other.m_lookup) { }
	constexpr SparseMap(SparseMap&& other) : m_array(std::move(other.m_array)), m_buckets(std::move(other.m_buckets)), m_lookup(std::move(other.m_lookup)) { }
	...
	void rehash(size_type count) noexcept {
		...
		for (size_type i = 0; i < m_array.size(); i++) {
			auto index = keyToBucket(m_array[i].first);

			m_lookup[i] = { index, m_buckets[index].size() };
			m_buckets[index].emplaceBack(i, m_array[i].first);
		}
	}
	...
	template <class K, class V> DEPRECATED constexpr pair_type<iterator, bool> insertOrAssign(const_iterator hint, K&& key, V&& value) noexcept {
		return insertOrAssign(std::forward<K>(key), std::forward<V>(value));
	}
	template <class K, class V> DEPRECATED constexpr pair_type<iterator, bool> insert_or_assign(const_iterator hint, K&& key, V&& value) noexcept {
		return insertOrAssign(std::forward<K>(key), std::forward<V>(value));
	}
	...
	template <class K, class... Args> DEPRECATED constexpr iterator tryEmplace(const_iterator hint, K&& key, Args&&... args) noexcept {
		auto it = basicFind(std::forward<K>(key));
	...
	template <class K, class... Args> DEPRECATED constexpr iterator try_emplace(const_iterator hint, K&& key, Args&&... args) noexcept {
	...
	constexpr iterator erase(iterator pos) noexcept {
		ASSERT(pos != m_array.end(), "lyra::SparseMap::erase(): The end iterator was passed to the function!");
	...
	constexpr iterator erase(const_iterator pos) noexcept {
		ASSERT(pos != em_array.nd(), "lyra::SparseMap::erase(): The end iterator was passed to the function!");
	...
	template <class K> NODISCARD constexpr bool contains(const K& key) const noexcept {
		...
		size_type size = bucketList.size();
		
		if (size > 1)
			for (; size > 0;)
				return (m_equal(bucketList[size--].second, key));
	...
	template <class K> NODISCARD constexpr mapped_type& at(const K& key) {
		auto it = basicFind(key);
		if (it == end()) throw std::out_of_range("lyra::SparseMap::at(): Specified key could not be found in container!");
		return it->second;
	}
	...
	template <class K> NODISCARD constexpr mapped_type& operator[](const K& key) {
		auto it = basicFind(key);
		return (it != m_array.end()) ? it->second : basicEmplace(key, mapped_type())->second;
	}
	template <class K> NODISCARD constexpr mapped_type& operator[](K&& key) {
		auto it = basicFind(std::forward<K>(key));
		return (it != m_array.end()) ? it->second : basicEmplace(std::forward<K>(key), mapped_type())->second;
	}
...
private:
	...
	constexpr iterator basicInsert(const value_type& value) noexcept {
		...
		m_buckets[i].emplaceBack(m_array.size(), value.first);
	...
	template <class K, class... Args> constexpr iterator basicEmplace(K&& key, Args&&... args) noexcept {
		...
		iterator it = &m_array.emplaceBack(std::forward<K>(key), std::forward<Args>(args)...);
